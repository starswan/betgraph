...
class CreatePrices < ActiveRecord::Migration[6.0]
  ...
  def change
    ...
    create_table(:prices, id: false, hypertable: hypertable_options) do |t|
      ...
      t.text :side, limit: 1, null: false
    ...
    MarketPrice.includes(:market_price_time).find_in_batches(batch_size: batch_size) do |mp_batch|
      ...
      say_with_time "MarketPrice #{index}/#{count} to Price #{percent}%" do
        batch = mp_batch.map do |mp|
          [
            [mp.back1price, mp.back1amount, "B", 1],
            [mp.lay1price, mp.lay1amount, "L", 1],
            [mp.back2price, mp.back2amount, "B", 2],
            [mp.lay2price, mp.lay2amount, "L", 2],
            [mp.back3price, mp.back3amount, "B", 3],
            [mp.lay3price, mp.lay3amount, "L", 3],
          ].reject { |z| z[0].nil? }.map do |price, amount, side, depth|
            { price: price,
              amount: amount,
              depth: depth,
