...
then
  ...
  ssh alice 'cd html/betgraph/current && . ~/.bash_profile && rvm use 3.1.6@bg && DISABLE_DATABASE_ENVIRONMENT_CHECK=1 rake db:drop db:create'
  date
# zcat/gzcat is confusing - gunzip -c (stdout) is a portable improvement
  time ssh alice "cd html/betgraph/current && . ~/.bash_profile && rvm use 3.1.6@bg && nice gunzip -c log/ssarbicity.sql.gz | rails db -p"
fi
