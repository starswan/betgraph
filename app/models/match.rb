...
class Match < ApplicationRecord
  ...
  scope :with_prices, -> { kept.where.not(prices_count: 0) }
  ...
  class << self
    def ransackable_attributes(_auth_object = nil)
      %w[actual_start_time bet_markets_count created_at date deleted_at division_id endtime half_time_duration id kickofftime live_priced prices_count name season_id type updated_at venue_id]
    end
  end
