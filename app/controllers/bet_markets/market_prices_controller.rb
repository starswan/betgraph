...
module BetMarkets
  class MarketPricesController < ApplicationController
    ...
    def index
      @market_prices = @bet_market.market_runners.map(&:market_prices).flatten
  ...
  private
    ...
    def find_bet_market
      @bet_market = BetMarket.includes(market_runners: { market_prices: :market_price_time }).find(params[:bet_market_id])
    end
  end
end
